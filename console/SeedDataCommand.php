...
use DB;
use Exception;
use Hts\Mall\Classes\Index\Index;
use Hts\Mall\Classes\Index\Noop;
use Hts\Mall\Classes\Index\ProductEntry;
use Hts\Mall\Classes\Index\VariantEntry;
use Hts\Mall\Updates\Seeders\DemoSeeder;
use Hts\Mall\Updates\Seeders\MallSeeder;
...
class SeedDataCommand extends Command
{
    ...
    {
        $question = 'All existing Hts.Mall data will be erased. Do you want to continue?';
        ...
        try {
            if (version_compare(System::VERSION, '3.0', '<')) {
                ...
                $this->callSilent('plugin:seed', [
                    'namespace' => 'Hts.Mall',
                    'class'     => 'Hts\Mall\Updates\Seeders\MallSeeder',
                ]);
            }
        ...
        if ($demo) {
            ...
            try {
                if (version_compare(System::VERSION, '3.0', '<')) {
                    ...
                    $this->callSilent('plugin:seed', [
                        'namespace' => 'Hts.Mall',
                        'class'     => 'Hts\Mall\Updates\Seeders\DemoSeeder',
                    ]);
                }
    ...
    protected function cleanup()
    {
        try {
            ...
            DB::table('system_files')
                ->where('attachment_type', 'LIKE', 'Hts%Mall%')
